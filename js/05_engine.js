...
function runCoreSimulation(cfg) {
    ...
    for (var run = 0; run < cfg.iterations; run++) {
        ...
        var log = function(time, evt, spell, res, dmg, castTime, info, mana) { var eclStr = ""; if(isNE()) eclStr="NAT"; if(isAE()) eclStr="ARC"; var mfRem = (State.activeMF && State.activeMF.exp > time) ? (State.activeMF.exp - time).toFixed(1) : "-"; var isRem = (State.activeIS && State.activeIS.exp > time) ? (State.activeIS.exp - time).toFixed(1) : "-"; var t3pStr = (State.t3End > time) ? (State.t3End - time).toFixed(1) : "-"; var t38Str = (State.t38End > time) ? (State.t38End - time).toFixed(1) : "-"; var ngStr = State.ng ? "YES" : "-"; var bBind = (State.bindingEnd > time) ? (State.bindingEnd - time).toFixed(1) : "-"; var bReos = (State.reosEnd > time) ? (State.reosEnd - time).toFixed(1) : "-"; var bToep = (State.toepEnd > time) ? (State.toepEnd - time).toFixed(1) : "-"; var bRoop = (State.roopEnd > time) ? (State.roopEnd - time).toFixed(1) : "-"; var bZhc = (State.zhcEnd > time) ? (State.zhcVal) : "-"; var oocStr = State.ooc ? "YES" : "-"; var boonStr = State.boon > 0 ? State.boon : "-"; var dispSP = getCurrentSP("Arcane"); var manaStr = (mana !== undefined) ? mana : "-"; RunLog.push({ t: time.toFixed(2), evt: evt, spell: spell, res: res, dmgNorm: dmg ? dmg.norm : 0, dmgEcl: dmg ? dmg.ecl : 0, dmgCrit: dmg ? dmg.crit : 0, castTime: castTime ? castTime + "s" : "-", ecl: eclStr, mfRem: mfRem, isRem: isRem, boat: State.boat, ng: ngStr, ooc: oocStr, boon: boonStr, sp: dispSP, t36: t3pStr, t38: t38Str, bBind: bBind, bReos: bReos, bToep: bToep, bRoop: bRoop, bZhc: bZhc, mana: manaStr, info: info || "", isAE: isAE(), isNE: isNE() }); };
