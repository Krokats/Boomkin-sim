/**
 * Moonkin Simulation - Community Builds (Supabase & Discord)
 */
...
async function publishBuild() {
    ...
    // TABELLE GEÄNDERT AUF MOONKIN
    const { data, error } = await supabaseClient
        .from('community_builds_boomkin')
        .insert([
            {
                type: type,
                title: title,
...
async function fetchCommunityBuilds(type) {
    ...
    if (type === 'gear' && typeof GEAR_PRESETS !== 'undefined') {
        Object.keys(GEAR_PRESETS).forEach(key => {
            systemBuilds.push({
                id: 'sys_' + key, type: type, title: key, comment: "Default Gear Setup",
                author_name: "System", author_id: "system", data: GEAR_PRESETS[key], isSystem: true
            });
        });
    } else if (type === 'talents' && typeof TALENT_PRESETS !== 'undefined') {
        Object.keys(TALENT_PRESETS).forEach(key => {
            systemBuilds.push({
                id: 'sys_' + key, type: type, title: key, comment: "Default Talent Build",
                author_name: "System", author_id: "system", data: TALENT_PRESETS[key], isSystem: true
            });
        });
        ...
        Object.keys(PRESET_ROTATIONS).forEach(key => {
            ...
            systemBuilds.push({
                id: 'sys_' + key, type: type, title: rot.name || key, comment: rot.desc || "Default Rotation Logic",
                author_name: "System", author_id: "system", data: rot, isSystem: true
            });
        });
    }
    ...
    if (error) {
        ...
        if (systemBuilds.length > 0) { renderCommunityBuilds(systemBuilds); } 
        else { tbody.innerHTML = `<tr><td colspan="3" class="text-center" style="color:red;">Error loading builds: ${error.message}</td></tr>`; }
        return;
    }
...
async function voteBuild(buildId, action) {
    if (!CURRENT_USER) { showCustomAlert("Error","You must be logged in with Discord to vote!"); return; }
    ...
    try {
        ...
        if (action === 'up') { if (!(data.upvoted_by || []).includes(uid)) upvotedBy.push(uid); } 
        else if (action === 'down') { if (!(data.downvoted_by || []).includes(uid)) downvotedBy.push(uid); }
...
function deleteCommunityBuild(buildId) {
    showCustomConfirm("Delete Build", "Are you sure you want to permanently delete this build? This action cannot be undone.", async () => {
        ...
        const { error } = await supabaseClient.from('community_builds_boomkin').delete().eq('id', buildId);
